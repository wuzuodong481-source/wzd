...
# -*- coding: utf-8 -*-
"""
move_arms_to_place.py — 双臂运动到【放置】姿态

使用方法:
  1. 修改下方 POSE_FILE 为要调用的位姿 JSON 文件路径
  2. 运行: python move_arms_to_place.py
...
#  ★ 可替换路径 - 调用的位姿 JSON 文件 ★
# ═══════════════════════════════════════════════════════════
POSE_FILE = "/data/wzd/position/pose_place.json"
# ═══════════════════════════════════════════════════════════
...
def main():
    parser = argparse.ArgumentParser(description="双臂运动到【放置】姿态")
    ...
    print(f"[放置] ✓ 已加载位姿 (采样时间: {ts})")
    print(f"[放置] 关节数: {len(joints)}")

...
# -*- coding: utf-8 -*-
"""
move_arms_to_pre_place.py — 双臂运动到【预放置】姿态

使用方法:
  1. 修改下方 POSE_FILE 为要调用的位姿 JSON 文件路径
  2. 运行: python move_arms_to_pre_place.py
...
#  ★ 可替换路径 - 调用的位姿 JSON 文件 ★
# ═══════════════════════════════════════════════════════════
POSE_FILE = "/data/wzd/position/pose_pre_place.json"
# ═══════════════════════════════════════════════════════════
...
def main():
    parser = argparse.ArgumentParser(description="双臂运动到【预放置】姿态")
    ...
    pose_path = args.pose
    print(f"[预放置] 位姿文件: {pose_path}")

    try:
        joints, ts = load_arm_joints(pose_path)
    except Exception as e:
        print(f"[预放置] ✗ 读取位姿失败: {e}")
        return 1

    print(f"[预放置] ✓ 已加载位姿 (采样时间: {ts})")
    print(f"[预放置] 关节数: {len(joints)}")

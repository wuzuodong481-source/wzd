...
def run_pick_correct(model, g2, target_depth, output_dir, dry_run=False):
    ...
    print(f"{'#'*60}")
    print(f"[纠偏] 模型: {os.path.basename(MODEL_PICK) if not cca.USE_RHINO_INFER else os.path.basename(cca.RHINO_REF_MODEL)}")
    print(f"[纠偏] 推理模式: {'辉羲RPU芯片' if cca.USE_RHINO_INFER else '远程MQTT' + ('+本地回退' if cca.REMOTE_INFER_FALLBACK else '')}")
...
def main():
    ...
    if (not args.skip_correct) and not dry:
        if cca.USE_RHINO_INFER:
            print(f"[初始化] 辉羲 RPU 芯片推理模式: {cca.RHINO_REF_MODEL}")
